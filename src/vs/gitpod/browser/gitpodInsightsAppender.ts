...
export class GitpodInsightsAppender implements ITelemetryAppender {
	...
	private async mapAnalytics(eventName: string, data: any) {
		...
		const mappedEvent = mapTelemetryData('window', eventName, data.properties);
		if (!mappedEvent) {
			return undefined;
		}
		...
		mappedEvent.properties = {
			...
			workspaceId,
			instanceId,
			debugWorkspace: debugWorkspaceType !== 'noDebug',
	...
	private _workspaceInfo: Promise<SupervisorWorkspaceInfo> | undefined;
		...
		if (!this._workspaceInfo) {
			this._workspaceInfo = (async () => {
				...
				return {
					...
					debugWorkspaceType: info.debugWorkspaceType || 'noDebug',
