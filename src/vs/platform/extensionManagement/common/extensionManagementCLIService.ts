...
export class ExtensionManagementCLIService implements IExtensionManagementCLIService {
	...
	public async installExtensions(extensions: (string | URI)[], builtinExtensionIds: string[], isMachineScoped: boolean, force: boolean, output: CLIOutput = console): Promise<void> {
		...
		if (vsixs.length) {
			await Promise.all(vsixs.map(async vsix => {
				try {
					const manifest = await this.installVSIX(vsix.uri, force, output, vsix.installOptions);
					if (manifest) {
	...
	private async installVSIX(vsix: URI, force: boolean, output: CLIOutput, options: InstallOptions): Promise<IExtensionManifest | null> {
		...
		if (valid) {
			try {
				await this.extensionManagementService.install(vsix, options);
