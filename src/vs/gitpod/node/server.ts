	...
	for (const pluginId in resolvedPlugins) {
		...
		pending.push((async () => {
			try {
				...
				await extensionManagementService.install(target, doNotSync);
			} catch (e) {
...
async function main(): Promise<void> {
	...
	const environmentService = new EnvironmentService(parsedArgs, process.execPath);
	...
	let resolveExtensionsInstalled: () => void;
