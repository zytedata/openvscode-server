...
import { buildHelpMessage, buildVersionMessage, ErrorReporter, OptionDescriptions, OPTIONS as ALL_OPTIONS } from 'vs/platform/environment/node/argv';
...
async function doMain<T extends ServerNativeParsedArgs = ServerNativeParsedArgs>(processArgv: string[], options: ServerCliOptions<T>): Promise<any> {
	...
	try {
		...
		args = options.parseArgs(processArgv.slice(2), errorReporter);
		if (args.goto) {
