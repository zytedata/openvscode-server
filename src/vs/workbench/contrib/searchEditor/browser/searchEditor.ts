...
export class SearchEditor extends BaseTextEditor {
	...
	private iterateThroughMatches(reverse: boolean) {
		...
		const matchRanges = this.getInput()?.getModel()?.getMatchRanges();
	...
	focusAllResults() {
		this.searchResultEditor
			.setSelections((this.getInput()?.getModel()?.getMatchRanges() ?? []).map(
	...
	private async doRunSearch() {
		...
		setTimeout(() => { throw Error('running search('); }, 100);
		...
		const bodyModel = await model.getResultTextModel();
		this.modelService.updateModel(bodyModel, results.text);
		model.setConfig(config);
		model.setMatchRanges(results.matchRanges);
	...
	async setInput(newInput: SearchEditorInput, options: EditorOptions | undefined, token: CancellationToken): Promise<void> {
		...
		const model = newInput.getModel();
		const bodyModel = await model.getResultTextModel();

		this.searchResultEditor.setModel(bodyModel);
		this.pauseSearching = true;

		const config = await model.getConfig();
		this.toggleRunAgainMessage(bodyModel.getLineCount() === 1 && bodyModel.getValue() === '' && config.query !== '');
